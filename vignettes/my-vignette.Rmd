...
custom_layer <- function(heatmap, arg){
  species <- sub('-.*', '', arg)
  pushViewport(viewport(x=0, width=unit(1, "in"), height=unit(1, "in"), just=c("left", "center")))
...
```{r, fig.height=7, fig.width=12, out.width='100%'}
...
for (i in 1:3){
  ...
  generate_pineplot(
    ms[[tissue]],
    filename = paste0('images/', tissue, '_pineplot.pdf'),

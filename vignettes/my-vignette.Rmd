...
custom_layer <- function(heatmap, arg){
  species <- sub('-.*', '', arg)
  pushViewport(viewport(x=0, y=.7, width=unit(1, "in"), height=unit(1, "in"), just=c("left", "center")))
...
pdf('example_2.pdf', width=11, height=7)
pushViewport(viewport(width=unit(10, 'in'), height=unit(7, 'in'), layout=grid.layout(1, 3)))
for (i in 1:3){

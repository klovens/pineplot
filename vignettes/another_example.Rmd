...
add_label <- function(panel, name){
  text <- paste0('Age:\n', sub('.*\\.', '', name))
  grid.text(text, x=0.1, y=.5, just='left')
}
```
...
```{r echo=FALSE, fig.height=7, fig.width=8, out.width='100%'}
...
for (sex in c(0, 1)){
  ...
  generate_pineplot(heart_by_sex, filename=paste0('images/sex-', sex, '_pineplot.pdf'),
